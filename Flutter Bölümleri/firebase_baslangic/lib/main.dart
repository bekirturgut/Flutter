...
void VeriEkleSet() async{
  ...
  await _firestore.doc("users/cBIFeAvcirQaKjhBKGfO").set({"Bölüm" : "Bilgisayar Mühendisliği"} , SetOptions(merge: true)); // merge true sayesinde verileri sıfırlamaz verilen bilgileri ekstra ekler veri kaybı yaşanmaz
}
void VeriGuncelle() async{
  await _firestore.doc("users/IeFcCmcbKFB9zfy6BSRo").update({"isim":"Güncel Bekir","yas":"22","ogrenciMi":false}); // verilen ıd li kullanıcının verilerini bu şekilde guncelleyebiliriz
...
void VeriSil() async{
  await _firestore.doc("users/cBIFeAvcirQaKjhBKGfO").update({"isim": FieldValue.delete()}); // verilen ıd deki kullanıcının isim verisini sildi
   await _firestore.doc("users/cBIFeAvcirQaKjhBKGfO").delete(); // verilen ıd deki kullanıcıyı tamamen sildi , ıd de kullanıcı yoksa hata vermiyor
}
void VeriOkuOneTime() async{
  ...
  debugPrint("BU KADAR VERİ VAR        --------${usersDocuments.size}---------           ");  // koleksiyonda kaç veri var onları yazdırırı.
